...
void printReadings(int n) {
  for (int i = 0; i < n; i++) {
    ...
    Serial.print("B01(A0)="); Serial.print(a0);
    Serial.print("  B02(A1)="); Serial.println(a1);
    delay(200);
  }
}
...
void loop() {
  ...
  setState("STBY on, FORWARD, PWM high (expect B01 high, B02 low)", true, true, false, true);
  printReadings(8);

  setState("STBY on, REVERSE, PWM high (expect B01 low, B02 high)", true, false, true, true);
  printReadings(8);
