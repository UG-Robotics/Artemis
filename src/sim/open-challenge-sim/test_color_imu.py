"""Score the ColorImuController in sim (uses the sim's noisy/drifting IMU +
...
def run_scored(widths, direction, start, seed, noise, p_spike=0.05):
    ...
    while elapsed < ROUND_TIME:
        ...
        if robot.check_wall_collision(track):
            ...
            in_coll = False
        elapsed += pdt
        if controller.state == CIState.STOPPED:
            break
    ...
    finish = 0
    if controller.state == CIState.STOPPED and laps >= 3 and elapsed < ROUND_TIME:
...
def main():
    print("ColorImuController — WRO open score (sim IMU+ToF, gyro-executed turns)\n")
